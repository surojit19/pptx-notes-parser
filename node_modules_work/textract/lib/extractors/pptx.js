  ...
  yauzl.open( filePath, function( err, zipfile ) {
    ...
    zipfile.on( 'end', function() {
      ...
      if ( slides.length ) {
        ...
        var tNotesText = slides.map( function( slide ) {                    
          return slide.tnotes;
        }).join( '\n' );

        
        tnotes = _calculateExtractedText( tNotesText );
    ...
    zipfile.on( 'entry', function( entry ) {
      //if ( slideMatch.test( entry.fileName ) ) {
        let slideText = slideMatch.test( entry.fileName ) ;
        let noteText = noteMatch.test( entry.fileName );
